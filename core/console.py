...
class ConsoleDemo:
    ...
    def search_card_info(self, card_num = None):
        ...
        stuCondition = NET_A_FIND_RECORD_ACCESSCTLCARD_CONDITION()
        stuCondition.dwSize = sizeof(NET_A_FIND_RECORD_ACCESSCTLCARD_CONDITION)
        if card_num:
            stuCondition.abCardNo = 1
            stuCondition.szCardNo = str(card_num).encode()
        inParam.pQueryCondition = cast(pointer(stuCondition), c_void_p)
        ...
        if result:
            ...
            outFindParam.nMaxRecordNum = 10
            recordInfoList = []
            for i in range(outFindParam.nMaxRecordNum):
                temp_stu = NET_RECORDSET_ACCESS_CTL_CARD()
                temp_stu.dwSize = sizeof(NET_RECORDSET_ACCESS_CTL_CARD)
                recordInfoList.append(temp_stu)
            outFindParam.pRecordList = cast(
                (NET_RECORDSET_ACCESS_CTL_CARD * outFindParam.nMaxRecordNum)(*recordInfoList), c_void_p)
            # 查找记录; search record
            result = self.sdk.FindNextRecord(inFindParam, outFindParam, 5000)
            if result:
                print("FindNextRecord operate success!")
                recordInfoList = cast(outFindParam.pRecordList,
                                      POINTER(NET_RECORDSET_ACCESS_CTL_CARD * outFindParam.nRetRecordNum)).contents
                for i in range(outFindParam.nRetRecordNum):
                    print("")
                    print("Record Info:" + str(i + 1))
                    print("RecNo. " + str(recordInfoList[i].nRecNo))
                    if card_num:
                        card_id = recordInfoList[i].nRecNo
                    print("CardNo. " + str(recordInfoList[i].szCardNo))
                    print("CardName. " + str(recordInfoList[i].szCardName, 'gbk'))
                    print("UserID. " + str(recordInfoList[i].szUserID))
    ...
    def user_operate(self):
        ...
        if result:
            print("OperateAccessCardService operate succeed.")
        else:
